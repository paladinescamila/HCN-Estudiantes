...
function TreatmentForm() {
  ...
  const { sectionInfo, sectionIndex } = GetSectionData("T");

  //Map the current activity to the fields in the form.
  const [currentActivityTreatment, setCurrentActivity] = useState({
    NutritionalPlanValue: sectionInfo.fields[0].value,
  });
  ...
  const findFormErrorsAndWarnings = () => {
    const { NutritionalPlanValue } = currentActivityTreatment;
    const Errors = {};
    const Warnings = {};

    if (!NutritionalPlanValue || NutritionalPlanValue === "") Warnings.NutritionalPlanValue = "El tratamiento del paciente";
        ...
        <Form style={{ width: "100%", paddingBottom: "15px" }}>
          <Form.Group>
            <Form.Control
              ...
              rows="10"
              name="NutritionalPlanValue"
              value={currentActivityTreatment.NutritionalPlanValue}
              label="Motivo de consulta"
              as="textarea"
              onChange={(e) => handleChange("NutritionalPlanValue", e.target.value)}
              isInvalid={!!errors.NutritionalPlanValue}
            />
            <Form.Control.Feedback className="form-input" type="invalid">
              {errors.NutritionalPlanValue}
