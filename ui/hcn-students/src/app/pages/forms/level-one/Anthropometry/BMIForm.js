...
import { errorMessages, isNum } from "../../shared/aids/Aids";
...
export function BMIForm() {
  ...
  const findFormErrorsAndWarnings = () => {
    const { BMIValue, BMIInter } = currentActivityAnthro;
    const Errors = {};
    const Warnings = {};

    if (!BMIValue || BMIValue === "") Warnings.BMIValue = "El valor del IMC";
    else if (isNum(BMIValue)) Errors.BMIValue = errorMessages.number;

    if (!BMIInter || BMIInter === "") Warnings.BMIInter = "La interpretación del IMC";
  ...
  return (
    <div style={{ paddingTop: "15px" }}>
      {HasHelpsCheck({ id: sectionInfo.fields[7].id, description: sectionInfo.fields[7].description, type: "text" }) ? (
        <OverlayTrigger
          placement="right"
          trigger="focus"
          overlay={OpenPropover({
            id: sectionInfo.fields[7].id,
            name: sectionInfo.fields[7].name,
            description: sectionInfo.fields[7].description,
          })}
          transition={false}
        >
          {({ ...triggerHandler }) => (
            <Button id="buttonTooltips" variant="none" {...triggerHandler} className="d-inline-flex">
              <h4 className="form-lvl-title">{sectionInfo.fields[7].name}</h4>
            </Button>
          )}
        </OverlayTrigger>
      ) : (
      ...
      <Form style={{ width: "100%", paddingBottom: "15px" }}>
          ...
          {HasHelpsCheck({
            ...
            <span style={{ paddingLeft: "5px" }}>
              <Fas
                ...
                onClick={() => handleImageHelp(sectionInfo.fields[7].name, sectionInfo.fields[7].id)}
              ></Fas>
            </span>
          ) : null}
          <Form.Control
            ...
            name="BMIValue"
            value={currentActivityAnthro.BMIValue}
            label="Valor"
            type="text"
            onChange={(e) => handleChange("BMIValue", e.target.value)}
            isInvalid={!!errors.BMIValue}
          />
        ...
        <Form.Group>
          ...
          <Form.Control
            className="form-input"
            name="BMIInter"
            value={currentActivityAnthro.BMIInter}
            label="Intepretación"
            as="textarea"
            onChange={(e) => handleChange("BMIInter", e.target.value)}
            isInvalid={!!errors.BMIInter}
          />
          <Form.Control.Feedback className="form-input" type="invalid">
            {errors.BMIInter}
          </Form.Control.Feedback>
